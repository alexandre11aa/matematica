...
if inicio == "T":
	...
	print()
	f4 = input("Informe a quarta derivada da função: ")
	print()
...
for k in range(n):
	...
	print("h = ({} - {}) / {} = {}".format(iv[1], iv[0], i, h), "\n")
        ...
        # Erro
	erro = (hP**5 / (80 * i**4)) * mf4
	print("O cálculo do erro será:", erro, "\n")
	print("E = ({}^5 / (80 * {}^4)) * {} = {}".format(hP, i, mf4, erro), "\n")
	...
	for j in range(len(x)):
		if   (j == 0):
			s += (3 * h / 8) * y[0]
			print("Aproximação = (3 * {} / 8) * {} + ".format(h, y[0]), end="")
		elif (j == (len(x) - 1)):
			s += (3 * h / 8) * y[len(x) - 1]
			print("(3 * {} / 8) * {} = {}".format(h, y[len(x) - 1], round(s,5)), "\n")			
		elif (j % 3 == 0) and (i > 4):
			s += (3 * h / 8) * 2 * y[j]
			print("(3 * {} / 8) * 2 * {} + ".format(h, y[j]), end="")               
		else:
			s += (3 * h / 8) * 3 * (y[j])
			print("(3 * {} / 8) * 3 * {} + ".format(h, y[j]), end="")
